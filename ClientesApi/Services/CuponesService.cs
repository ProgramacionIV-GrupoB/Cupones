...
namespace ClientesApi.Services
{
    public class CuponesService : ICuponesService
    {
        ...
        public async Task<string> ObtenerCuponesActivos(string codigoCliente)
        {
            try
            {
                using var client = new HttpClient();
                var respuesta = await client.GetAsync($"https://localhost:7269/api/Cupones/cliente/{codigoCliente}");

                if (respuesta.IsSuccessStatusCode)
                {
                    var cuponesJson = await respuesta.Content.ReadAsStringAsync();

                    var cuponesObjeto = JsonConvert.DeserializeObject(cuponesJson);
                    var cuponesFormateado = JsonConvert.SerializeObject(cuponesObjeto, Formatting.Indented);

                    return cuponesFormateado;
                }
                else
                {
                    var error = await respuesta.Content.ReadAsStringAsync();
